...
# Loop through each region and train the model
for region in regions:
    ...
    # Print the results for this region
...
# Drop target and ID columns, keeping the columns defined above for features
X = df.drop(columns=["Price", "Property ID"])  # Drop target and ID columns
y = df["Price"]  # Target variable

# Define subsets of columns for each regression
...
categorical_transformer = Pipeline(steps=[
    ("imputer", SimpleImputer(strategy="constant", fill_value="Missing")),  # Treat missing values as 'Missing' category
    ("onehot", OneHotEncoder(sparse_output=False, handle_unknown='ignore')),  # One-Hot Encoding with 'Unknown_' handling
    ("scaler", StandardScaler())  # Standardize one-hot encoded features
])

# Preprocessing for numerical variables: Standardization (excluding Latitude and Longitude)
numerical_transformer = Pipeline(steps=[
    ("scaler", StandardScaler())
])

# Preprocessing for geographical variables: No Standardization
geographical_transformer = Pipeline(steps=[
    ("identity", "passthrough")  # Keep Latitude and Longitude as-is
])

# KNN and Random Forest for Categorical Variables only
X_cat = X[categorical_cols]

X_train, X_test, y_train, y_test = train_test_split(X_cat, y, random_state=42)

# Full pipeline for KNN Regressor (only Categorical variables)
knn_cat_pipeline = Pipeline(steps=[
    ("preprocessor", ColumnTransformer(transformers=[("cat", categorical_transformer, categorical_cols)])),
    ("knn", KNeighborsRegressor())  
])

# Full pipeline for Random Forest Regressor (only Categorical variables)
rf_cat_pipeline = Pipeline(steps=[
    ("preprocessor", ColumnTransformer(transformers=[("cat", categorical_transformer, categorical_cols)])),
    ("rf", RandomForestRegressor(n_estimators=100, random_state=42))
])

# Set up the parameter grid for GridSearchCV to find the optimal number of neighbors (k)
param_grid_knn = {
...
# Fit and evaluate KNN for Categorical Variables using GridSearchCV
grid_search_knn.fit(X_train, y_train)

# Get the best KNN model (with the best value for n_neighbors)
best_knn_cat = grid_search_knn.best_estimator_

# Make predictions and evaluate performance for KNN
...
# Fit and evaluate Random Forest for Categorical Variables
...
# KNN and Random Forest for Numerical Variables only
X_num = X[numerical_cols].copy()  
X_num = X_num.dropna()

# Ensure that the target variable corresponds to the cleaned data (X_num)
y_num = y[X_num.index]

X_train, X_test, y_train, y_test = train_test_split(X_num, y_num, random_state=42)

# Full pipeline for KNN Regressor (only Numerical variables)
knn_num_pipeline = Pipeline(steps=[
    ("preprocessor", ColumnTransformer(transformers=[("num", numerical_transformer, numerical_cols)])),
    ("knn", KNeighborsRegressor())  
])

# Full pipeline for Random Forest Regressor (only Numerical variables)
...
# Fit and evaluate KNN for Numerical Variables using GridSearchCV
...
# Fit and evaluate Random Forest for Numerical Variables
...
# KNN and Random Forest for All Variables (with dropna for numerical columns)
X_all = X.copy()
...
# Full pipeline for KNN Regressor (all variables)
...
# Full pipeline for Random Forest Regressor (all variables)
...
# Fit and evaluate KNN for All Variables using GridSearchCV
...
# Fit and evaluate Random Forest for all variables
...
# List of unique Address Regions (assuming 'Address Region' is a column in your dataframe)
regions = X["Address Region"].unique()

# Iterate through each region and perform the KNN and Random Forest regressions
for region in regions:
    ...
    # Stratified train-test split by 'Address Region' (though this is redundant within each region loop)
...
# Group by region and feature type to make the table more readable
